 ...
 */
void usys_putc(char ch) { syscall(SYS_putc, ch, 0, 0, 0, 0, 0, 0, 0, 0); }

void usys_exit(int ret) { syscall(SYS_exit, ret, 0, 0, 0, 0, 0, 0, 0, 0); }
...
int usys_map_pmo(u64 process_cap, u64 pmo_cap, u64 addr, u64 rights) {
  return (int)syscall(SYS_map_pmo, process_cap, pmo_cap, addr, rights, 0, 0, 0,
                      0, 0);
}
...
u64 usys_yield(void) { return syscall(SYS_yield, 0, 0, 0, 0, 0, 0, 0, 0, 0); }
...
u64 usys_register_server(u64 callback, u64 max_client, u64 vm_config_ptr) {
  return syscall(SYS_register_server, callback, max_client, vm_config_ptr, 0, 0,
                 0, 0, 0, 0);
}

u32 usys_register_client(u32 server_cap, u64 vm_config_ptr) {
  return syscall(SYS_register_client, server_cap, vm_config_ptr, 0, 0, 0, 0, 0,
                 0, 0);
}
...
int usys_debug(void) { return syscall(SYS_debug, 0, 0, 0, 0, 0, 0, 0, 0, 0); }

int usys_cap_copy_to(u64 dest_process_cap, u64 src_slot_id) {
  return syscall(SYS_cap_copy_to, dest_process_cap, src_slot_id, 0, 0, 0, 0, 0,
                 0, 0);
}

int usys_cap_copy_from(u64 src_process_cap, u64 src_slot_id) {
  return syscall(SYS_cap_copy_from, src_process_cap, src_slot_id, 0, 0, 0, 0, 0,
                 0, 0);
}
...
void usys_top(void) { syscall(SYS_top, 0, 0, 0, 0, 0, 0, 0, 0, 0); }
