...
int handle_trans_fault(struct vmspace *vmspace, vaddr_t fault_addr) {
  ...
  if (!vmr || vmr->pmo->type != PMO_ANONYM) return -ENOMAPPING;

  void *new_page = kzalloc(PAGE_SIZE);
  if (!new_page) return -ENOMAPPING;

  vmr->pmo->size += PAGE_SIZE;
  if (map_range_in_pgtbl(vmspace->pgtbl, ROUND_DOWN(fault_addr, PAGE_SIZE),
                         virt_to_phys(new_page), PAGE_SIZE, vmr->perm) < 0) {
    return -ENOMAPPING;
  }
