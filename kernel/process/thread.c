...
static u64 load_binary(struct process *process, struct vmspace *vmspace,
  ...
  for (i = 0; i < elf->header.e_phnum; ++i) {
    ...
    if (elf->p_headers[i].p_type == PT_LOAD) {
       ...
       * page aligned segment size. Take care of the page alignment when
       * allocating and mapping physical memory.
       */
       ...
       */
      u64 file_sz = elf->p_headers[i].p_filesz;
      void *dst = (void *)phys_to_virt(pmo->start) + p_vaddr % PAGE_SIZE;
      memcpy(dst, elf->p_headers[i].p_offset + bin, file_sz);
      memset(dst + file_sz, 0, seg_sz - file_sz);
...
int thread_create_main(struct process *process, u64 stack_base, u64 stack_size,
  ...
  prepare_env((char *)phys_to_virt(stack_pmo->start) + stack_size, stack, &meta,
              bin_name);
...
int sys_set_affinity(u64 thread_cap, s32 aff)
{
  ...
  if(thread_cap != -1) 
    obj_put(thread); // match to obj_get
